    ...
    if request.method == "POST":
        if form.is_valid():
            ...
            if not Team.objects.filter(team_num=team_code).exists():
                ...
                VID = str("".join(random.choices(string.digits, k=7)))
...
def teamManagement(request):
    if Pit_stats.objects.filter(team_num=request.user.team_num).exists():
        context = {
            ...
            "usersCount": CustomUser.objects.filter(
                team_num=request.user.team_num
            ).count(),
            "is_incorrect": Pit_stats.objects.get(
                team_num=CustomUser.objects.get(username=request.user.username).team_num
            ).is_incorrect,
            "is_hidden": Pit_stats.objects.get(
                team_num=CustomUser.objects.get(username=request.user.username).team_num
            ).is_hidden,
            "link": Pit_stats.objects.get(
                team_num=CustomUser.objects.get(username=request.user.username).team_num
            ).pk,
        }
    else:
